...
class MemoryManager:
    ...
    def __init__(self, chromadb_url: str, redis_url: str, db_pool, ollama_url: str):
        ...
        self.memory_consolidator.consolidation_thread = threading.Thread(
            target=self._run_consolidation_job,
            daemon=True
        )
    ...
    def _run_consolidation_job(self):
        """Run consolidation job with proper database access"""
        while True:
            try:
                if not self.db_pool:
                    logger.error("Database pool not initialized")
                ...
                for user in users:
                    result = self.memory_consolidator.consolidate_old_memories(user, cutoff_days=7)
                    if result['messages_consolidated'] > 0:
                        logger.info(f"Consolidated {result['messages_consolidated']} messages for {user}")
